...
public class Reload {
	private static float time_prim_1 = 0;
	private static float time_prim_2 = 0;
	private static float time_sec1 = 0;
	private static float time_sec2 = 0;

	public static void reload_primary_1(Planes pln, float delta) {
		WeaponTypes wpn = pln.getWeapon(1);
		if (pln.getAmmo(1) == 0) {
			time_prim_1 += delta;
			if (time_prim_1 >= wpn.getReload_delay()) {
	...
	public static void reload_primary_2(Planes pln, float delta) {
		WeaponTypes wpn = pln.getWeapon(2);
		if (pln.getAmmo(2) == 0) {
			time_prim_2 += delta;
			if (time_prim_2 >= wpn.getReload_delay()) {
	...
	public static void reload_secondary_2(Planes pln, float delta) {
		WeaponTypes wpn = pln.getWeapon(4);
		if (pln.getAmmo(4) == 0) {
			time_sec2 += delta;
